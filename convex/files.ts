...
export const createFile = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if(existing) throw new Error("File already exist")
...
export const renameFile = mutation({
    ...
    handler: async(ctx, args) => {
        ...
        const existing = sibilings.find(
            (sibilings) => 
                sibilings.name === args.newName &&
                sibilings.type === file.type &&
                sibilings._id !== args.id
        ...
        await ctx.db.patch("files", args.id,{name: args.newName, updatedAt: Date.now()})
